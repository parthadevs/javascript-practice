# 📚 JavaScript Practice Repository

A structured repository for learning and practicing modern JavaScript through theory, coding exercises, workshops, tricky questions, and revision.

---

## 📂 Repository Structure

```text
javascript-practice
├── js-array
├── js-date
├── js-function
├── js-loop
├── js-numbers
├── js-object
├── js-string
├── js-tricky-questions
├── revision
├── workshop
└── README.md
```

---

## 🚀 Learning Progress Tracker

| Topic            |     Days    | Practiced Files / Source Code                                                                                                                                                                                                                                                                                                                      |
| :--------------- | :---------: | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **JS String**    |    Day 1    | 📁 [methods.js](./js-string/methods.js) <br> 📁 [search.js](./js-string/search.js) <br> 📁 [string.js](./js-string/string.js) <br> 📁 [regular-expressions.js](./js-string/regular-expressions.js) <br> 📁 [string-searching-method-practice.js](./js-string/string-searching-method-practice.js)                                                  |
| **JS Numbers**   |    Day 2    | 📁 [methods.js](./js-numbers/methods.js) <br> 📁 [properties.js](./js-numbers/properties.js) <br> 📁 [bigint.js](./js-numbers/bigint.js) <br> 📁 [bitwise.js](./js-numbers/bitwise.js)                                                                                                                                                             |
| **JS Functions** |    Day 3    | 📁 [function.js](./js-function/function.js)                                                                                                                                                                                                                                                                                                        |
| **JS Arrays**    |   Day 4-6   | 📁 [array.js](./js-array/array.js) <br> 📁 [methods.js](./js-array/methods.js) <br> 📁 [shallow-copy.js](./js-array/shallow-copy.js) <br> 📁 [shopping-list.js](./js-array/shopping-list.js) <br> 📁 [golf-score-translator.js](./js-array/golf-score-translator.js) <br> 📁 [lab-lunch-picker-program.js](./js-array/lab-lunch-picker-program.js) |
| **JS Objects**   |   Day 6-8   | 📁 [object.js](./js-object/object.js) <br> 📁 [properties.js](./js-object/properties.js) <br> 📁 [json.js](./js-object/json.js)                                                                                                                                                                                                                    |
| **JS Loops**     |   Day 8-10  | 📁 [loop.js](./js-loop/loop.js)                                                                                                                                                                                                                                                                                                                    |
| **JS Date**      | Coming Soon | 📁 `js-date/`                                                                                                                                                                                                                                                                                                                                      |

---

## 🧪 JavaScript Workshops

| Workshop                    | Concepts Practiced                             | Source Code                                                                  |
| :-------------------------- | :--------------------------------------------- | :--------------------------------------------------------------------------- |
| Cargo Manifest Validator | Objects, Validation, Functions                 | 📁 [cargo-manifest-validator.js](./workshop/cargo-manifest-validator.js)     |
| Quiz Game                | Objects, Arrays, Random Selection              | 📁 [quiz-game.js](./workshop/quiz-game.js)                                   |
| Recipe Tracker           | Objects, Arrays, Functions                     | 📁 [recipe-tracker.js](./workshop/recipe-tracker.js)                         |
| Record Collection        | Objects, Arrays, CRUD Operations               | 📁 [record-collection.js](./workshop/record-collection.js)                   |
| Space Mission Roster     | Arrays, Objects, Sorting, Validation, Chunking | 📁 [space-mission-roster.js](./workshop/space-mission-roster.js)             |
| Traffic Light Sequencer  | Loops, State Management                        | 📁 [traffic-light-sequencer.js](./workshop/traffic-light-sequencer.js)       |
| Sentence Analyzer        | Loops, String Processing                       | 📁 [workshop-sentence-analyzer.js](./workshop/workshop-sentence-analyzer.js) |
| Word Counter             | Loops, String Processing                       | 📁 [workshop-word-counter.js](./workshop/workshop-word-counter.js)           |
| Longest Word Finder             | Loops, String Processing                       | 📁 [longest-word-finder.js](./workshop/longest-word-finder.js)           |
| Heritage Library Catalog | Objects, Arrays, String Processing, Data Parsing, Validation, Search, JSON, CSV Export | 📁 [heritage-library-catalog.js](./workshop/heritage-library-catalog.js) |
---

## 🧩 JavaScript Tricky Questions

| Category   | Files                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| :--------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **String** | 📁 [cache-illusion.js](./js-tricky-questions/string/cache-illusion.js) <br> 📁 [emoji-length.js](./js-tricky-questions/string/emoji-length.js) <br> 📁 [empty-array-concatenation.js](./js-tricky-questions/string/empty-array-concatenation.js) <br> 📁 [immutability-and-memory.js](./js-tricky-questions/string/immutability-and-memory.js) <br> 📁 [others.js](./js-tricky-questions/string/others.js) <br> 📁 [sneaky-substring.js](./js-tricky-questions/string/sneaky-substring.js) |

---

## 🔄 Revision

| Topic             | Files                                                                                                                                                                      |
| :---------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Revision Practice | 📁 [rev.assignment-operator.js](./revision/rev.assignment-operator.js) <br> 📁 [rev.search.js](./revision/rev.search.js) <br> 📁 [rev.string.js](./revision/rev.string.js) |

---

## 🎯 Learning Goals

* Master JavaScript Fundamentals
* Understand Arrays, Objects, Functions, and Loops
* Practice String & Number Manipulation
* Learn Regular Expressions
* Solve Real-world JavaScript Workshops
* Improve Problem Solving Skills
* Build Strong JavaScript Foundations for DSA & Backend Development
* Maintain Daily Coding Consistency

---

## 🛠️ Tech Stack

* JavaScript (ES6+)
* Node.js
* Git & GitHub
* VS Code
