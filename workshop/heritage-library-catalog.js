...
Description
-----------
A JavaScript workshop that simulates a digital library catalog system.
The project reads raw catalog card data, converts it into structured objects,
validates records, groups books by decade, searches by author, renders library
cards, and exports the catalog in JSON and CSV formats.

Workshop Features
-----------------
✓ Parse raw catalog card strings into JavaScript objects
✓ Normalize and clean imported data
✓ Handle missing or invalid values with fallback defaults
...
- Learn data parsing and normalization
...
for (const entry of catalog) {
  if (entry.year !== "Unknown") {
    oldestYear = Math.min(oldestYear, entry.year);
    newestYear = Math.max(newestYear, entry.year);
  }
}
